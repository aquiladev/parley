...
The repo ships a logo pack at `/artifacts/`. The full inventory and "where to use what" table live in `artifacts/README.md` — read that before adding any new branded surface.

**Source of truth:** `/artifacts/svg/*.svg` (parametric, generated by `artifacts/build.py`). Don't hand-edit derived PNGs/ICOs/favicons; regenerate or copy from `artifacts/`.

**Conventions to honor when adding any branded surface:**

- **Mini App** — favicons, manifest, OG cards, and the lockup live under `packages/miniapp/public/{favicon,social,lockup-horizontal.svg,mark.svg}`. They are mirrored from `artifacts/` via `pnpm -F @parley/miniapp sync-assets`. **Never hand-edit files under `packages/miniapp/public/` for branding.** If you change a logo, edit the SVG source in `artifacts/svg/`, regenerate via `artifacts/build.py`, then rerun `sync-assets`. Every Mini App page renders `<MiniAppHeader />` from `lib/header.tsx` for consistency.
- **README hero** — `<picture>` switch between `artifacts/svg/lockup-stacked.svg` (light) and `artifacts/svg/mark-on-dark.svg` (dark). Don't rely on `currentColor` for inline `<img>` SVGs in markdown — GitHub doesn't propagate page color across the boundary. Use the `<picture>` pattern when a surface needs theme-aware rendering.
- **Light/dark theme awareness** — favicon link tags use `media="(prefers-color-scheme: ...)"` to switch SVG variants. Carry this convention to any new HTML surface.
- **Telegram bot avatar** — set externally via @BotFather to `artifacts/png/avatar-dark-512.png`. Note in deployment notes when first applied; not a code change.
- **GitHub repo icon / social card** — set in repo settings to `artifacts/png/app-icon-light-256.png`. Same — external, not a code change.
- **Future asset additions** — when introducing a new branded context (a new pitch deck slide, sponsor card, embed widget, etc.), grep `artifacts/README.md` for an existing asset that fits before generating a new one. Avoid asset proliferation; the pack is monochrome by deliberate restraint (see the README's "Color" section).

If you find a place that should have a logo but doesn't (a new web surface, a new doc with branding, an OG meta tag missing), fix it inline rather than leaving it as a TODO — branding is cheap to add when in flight, expensive to retrofit.
